...
def main(config):
    config.trainer.nnodes = int(config.trainer.nnodes)
    run_ppo(config)
...
def run_ppo(config) -> None:
    ...
    if not ray.is_initialized():
        ...
        print("[INFO] Forcing Ray into local mode...,num_gpus: ",gpu_num)
        ray.init(
            address=None,  
            ...
            num_cpus=config.get("ray_init", {}).get("num_cpus", gpu_num),
        )
